...
predictRT = function(input,
  ...
  datinput = datinput %>% 
      rowwise() %>%
      mutate(mols1 = tryCatch(parse.smiles(DnsCl_SMILES), 
                              error = function(e) NULL)) %>%
      mutate(mols2 = tryCatch(parse.smiles(MPEA_SMILES), 
                              error = function(e) NULL))
    datinput = datinput %>% 
    rowwise() %>%
    mutate(mols1 = tryCatch(parse.smiles(DnsCl_SMILES), 
  ...
  #make condition
  if (c('mols1') %in% names(datinput) && c('mols2') %in% names(datinput)){
    datinput2 = datinput %>%
      mutate(DnsCl= case_when(
    ...
    datinput = datinput %>%
      left_join(predrt1, by='DnsCl_SMILES')
    newoutput = datinput %>%
      left_join(predrt2, by='MPEA_SMILES') %>%
      distinct()
    #print(head(newoutput))
  } else if(c('mols1') %in% names(datinput)){
    #rows only with DnsCl
    datinput_1 = datinput %>%
      select(-c(SMILES))%>%         #remove original smiles
      rename(SMILES = DnsCl_SMILES)  #rename derivatized smiles as SMILES for RTprediction
    predrt1 = as.data.frame(predictionRT(datinput_1,
                                         ...
                                         filpath))
    #print(head(predrt1))
    #combine data frame of DnsCl_RT and MPEA_RT
    datinput = datinput %>%
      mutate_at(c('DnsCl_SMILES'), ~na_if(., ''))%>%
      select(-contains('mol')) %>%       #drop mols1 and mols2
      left_join(predrt1, by='DnsCl_SMILES')    
    ##mutate the MPEA_RT column
    newoutput = datinput %>%
      mutate(MPEA_RT = NA) %>%
      distinct()
    #print(head(newoutput))
